        ...
        {   input =>
            new Iterator[Row] {
                ...
                def next() = {
                    ...
                    if (cache.containsKey(cacheKeyProjection.apply(nextRow)))
                        cache.get(cacheKeyProjection.apply(nextRow))
                    else {
                        val cacheKey = cacheKeyProjection.apply(nextRow)
                        val preRow = preUdfProjection.apply(nextRow)
                        val udfRow = udfProject.apply(nextRow)
                        val postRow = postUdfProjection.apply(nextRow)
                        val concatRow: Row = Row.fromSeq(preRow ++ udfRow ++ postRow)
                        cache.put(cacheKey, concatRow)
                        concatRow
                    }
                }
            }

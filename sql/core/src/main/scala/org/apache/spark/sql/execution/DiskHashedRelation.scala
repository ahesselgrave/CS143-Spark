...
import org.apache.spark.sql.catalyst.expressions.{MutableRow, Projection, Row}
...
private[sql] class DiskPartition (
    ...
    private val block_size: Int = blockSize
	/**
            ...
            private[this] def fetchNextChunk(): Boolean = {
                if (chunkSizeIterator.hasNext) {
                    ...
                    if (chunkSize == 0)
                        ...
                        val nextChunkBytes: Array[Byte] = new Array[Byte](chunkSize)
                        inStream.read(nextChunkBytes, 0, chunkSize)
